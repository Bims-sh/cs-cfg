...
newContent=$(echo "$content" | sed 's/"setting.lb_shadow_texture_width_override"[[:space:]]\+"[0-9]\+"/"setting.lb_shadow_texture_width_override"		"2048"/')
newContent=$(echo "$newContent" | sed 's/"setting.lb_shadow_texture_height_override"[[:space:]]\+"[0-9]\+"/"setting.lb_shadow_texture_height_override"		"3072"/')
newContent=$(echo "$newContent" | sed 's/"setting.csm_max_shadow_dist_override"[[:space:]]\+"[0-9]\+"/"setting.csm_max_shadow_dist_override"		"560"/')
newContent=$(echo "$newContent" | sed 's/"setting.lb_csm_cascade_size_override"[[:space:]]\+"[0-9]\+"/"setting.lb_csm_cascade_size_override"		"640"/')
